...
handle.userHandler = (req, callback) => {
  ...
  if (acceptedMethod.includes(req.method)) {
    handle.user[req.method](req, callback);
  } else {
    callback(405);
  }
};
...
handle.user.get = (req, callback) => {
  const phone = typeof req.queryStringObj.phone === 'string'
        && req.queryStringObj.phone.trim().length === 11
    ? req.queryStringObj.phone
    : false;

  if (phone) {
    const token = typeof req.headersObj.token === 'string' && req.headersObj.token?.trim().length === 20
      ? req.headersObj.token
      : false;
    if (token) {
      tokenHandler.token.verify(token, phone, (valid) => {
        if (valid) {
...
handle.user.post = (req, callback) => {
  const firstname = typeof req.body.firstname === 'string' && req.body.firstname.trim().length
    ? req.body.firstname
    : false;
  const lastname = typeof req.body.lastname === 'string' && req.body.lastname.trim().length
    ? req.body.lastname
    : false;
  ...
  const password = typeof req.body.password === 'string' && req.body.password.trim().length
    ? req.body.password
    : false;
  const tosAgreement = typeof req.body.tosAgreement === 'boolean' && req.body.tosAgreement
...
handle.user.delete = (req, callback) => {
  const phone = typeof req.queryStringObj.phone === 'string'
        && req.queryStringObj.phone.trim().length === 11
    ? req.queryStringObj.phone
    : false;
