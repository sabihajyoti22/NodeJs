...
handle.tokenHandler = (req, callback) => {
    ...
    if (acceptedMethod.includes(req.method)) {
        handle._token[req.method](req, callback);
    } else {
        callback(405);
    }
};
...
handle._token.get = (req, callback) => {
    const id =
        typeof req.queryStringObj.id === 'string' && req.queryStringObj.id.trim().length === 20
...
handle._token.post = (req, callback) => {
    const phone =
...
handle._token.put = (req, callback) => {
    const id =
        typeof req.body.id === 'string' && req.body.id.trim().length === 20 ? req.body.id : false;
...
handle._token.delete = (req, callback) => {
